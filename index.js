...
app.get('/movies/genres/:genreName', (req, res) => {
  let genre = movies.find((movie) => { return movie.genre.genreName === req.params.genreName });
...
app.get('/movies/directors/:directorName', (req, res) => {
  let director = movies.find((movie) => { return movie.director.directorName === req.params.directorName });
...
app.post('/users/:id/:favMovie', (req, res) => {
  let user = users.find((user) => { return user.id === parseInt(req.params.id) });
  let favMovie = req.params.favMovie;

  if (user) {
    user.favouriteMovies.push(favMovie);
    res.status(200).send(favMovie + " has been added to " + user.name + "\'s favourites films\n\n" + JSON.stringify(users));
  } else {
    res.status(400).send('There is no such user')
  }
});
...
app.delete('/users/:id/:favMovie', (req, res) => {
  let user = users.find((user) => { return user.id === parseInt(req.params.id) });
  let favMovie = req.params.favMovie;

  if (user) {
    if (user.favouriteMovies.includes(favMovie)) {
      user.favouriteMovies.splice(user.favouriteMovies.indexOf(favMovie), 1);
      res.status(200).send(favMovie + " has been removed from " + user.name + "\'s favourites films\n\n" + JSON.stringify(users));
    } else {
      res.status(400).send('There is no movie in favourites')
    }
  } else {
    res.status(400).send('There is no such user')
  }
...
app.delete('/users/:id', (req, res) => {
  ...
  if (user) {
    ...
    res.status(201).send(user.name + "(" + user.email + ")" + " has been deleted\n\n" + JSON.stringify(users));
  } else {
    res.status(400).send('There is no such user')
  }
});
